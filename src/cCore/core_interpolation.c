...
int Add_i3Dlin( int owner, double* pointer, int size, int components, 
...
void i3Dlin( circuit *c )
 {
    if (c->iparams[5] != 0){
        ...
        for (CompCounter=0;CompCounter<c->iparams[1];CompCounter++){
			...
			int i = xo-xmin; //CHECK: xo is double, xmin is double, their difference is double
			int j = yo-ymin; //but you store it as an integer! this is not always predictable!
			int k = zo-zmin;
			...
			//CHECK: same here! gridstepx is a double, and u sum it with an int
			//then store the result in tracker which is int!!! this will not work!
			double Fioo = c->params[tracker]; //the indexes could all be wrong!
